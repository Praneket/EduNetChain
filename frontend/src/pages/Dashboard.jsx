...
import { getPosts, applyToPost, likePost, getComments, addComment, getSkillGap, getRecommendations, getAlumniList, getMyApplications, getMyProfile } from "../api";
...
export default function Dashboard() {
  ...
  useEffect(() => { fetchAI(); fetchProfileViews(); }, []);
  useEffect(() => { loadAllAlumni(); }, []);

  const fetchProfileViews = async () => {
    try {
      const res = await getMyProfile();
      setProfileViews(res.data.profileViews || 0);
    } catch {}
  };
  ...
  const fetchAI = async () => {
    try {
      ...
      setRecommendations(recRes.data.recommendations?.filter(r => r.matchScore > 0) || []);
    } catch {}
  };
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="border-t border-gray-100 px-4 py-3 space-y-2">
              <div className="flex justify-between text-xs">
                ...
                <span className="text-[#0a66c2] font-semibold">{profileViews !== null ? profileViews : '—'}</span>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-2">
                {recommendations.slice(0, 4).map(r => (
                  <div key={r._id} className="p-2 bg-blue-50 rounded-lg border border-blue-100">
                    ...
                    <p className="text-xs text-green-600 font-medium mt-0.5">{r.matchScore}% match</p>
                  </div>
                ))}
              </div>
            </div>
