...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    fetchConversation(selectedUser.userId, true); // show loader on first open
    const interval = setInterval(() => fetchConversation(selectedUser.userId), 5000); // silent poll
  ...
  const fetchConversation = async (userId, showLoader = false) => {
    try {
      if (showLoader) setLoading(true);
      const res = await getConversation(userId);
      setMessages(res.data);
      fetchInbox();
    } catch {} finally { if (showLoader) setLoading(false); }
  };
