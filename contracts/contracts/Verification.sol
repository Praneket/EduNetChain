...
contract Verification {
    ...
    // ── Versioned hash (UPDATE flow) ─────────────────────────────────────────
    struct VersionedHash {
        bytes32 hash;
        bytes32 previousHash; // 0x0 for first version
        uint256 timestamp;
    }

    mapping(address => VersionedHash[]) private versionedRecords;

    event VersionedHashStored(address indexed user, bytes32 hash, bytes32 previousHash, uint256 timestamp);

    function storeVersionedHash(address user, bytes32 hash, bytes32 previousHash) external onlyOwner {
        versionedRecords[user].push(VersionedHash(hash, previousHash, block.timestamp));
        records[user].push(hash);
        emit VersionedHashStored(user, hash, previousHash, block.timestamp);
    }

    function getVersionedRecords(address user) external view returns (VersionedHash[] memory) {
        return versionedRecords[user];
    }
