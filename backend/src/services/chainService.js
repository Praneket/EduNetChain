...
const abi = [
  'function storeVersionedHash(address user, bytes32 hash, bytes32 previousHash) external',
  'function getVersionedRecords(address user) external view returns (tuple(bytes32 hash, bytes32 previousHash, uint256 timestamp)[] memory)',
...
async function storeVersionedHash(userAddress, hash, previousHash) {
  try {
    const prev = previousHash || ('0x' + '0'.repeat(64));
    const tx = await contract.storeVersionedHash(userAddress, hash, prev);
    const receipt = await tx.wait();
    return receipt.hash;
  } catch (err) {
    console.error('❌ storeVersionedHash error:', err.message);
    throw err;
  }
}

module.exports = { issueCredential, storeOnChain, storeVersionedHash, storePostHashOnChain, getVerifications, verifyHash, verifyPostHash };
