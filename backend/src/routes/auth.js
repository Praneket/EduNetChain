...
const resumeUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    path.extname(file.originalname).toLowerCase() === '.pdf'
      ? cb(null, true) : cb(new Error('Only PDF allowed for resume'));
  },
});
...
router.post('/register', loginLimiter, upload.fields([
  ...
  try {
    ...
    const certPaths = (await Promise.all(certFiles.map(f => uploadToSupabase(f)))).filter(Boolean);
    ...
    if ((role || 'student') === 'student') {
      try {
        await Request.create({
          studentId: user._id,
          type: 'ADD',
          data: { name, email, educationInfo, skills },
        });
      } catch (reqErr) {
        console.error('Request.create error (non-fatal):', reqErr.message);
      }
    }

    res.status(201).json({ msg: 'Registration successful. Await validator verification.' });
  } catch (err) {
...
router.post('/login', loginLimiter, async (req, res) => {
  try {
    ...
    if (!user.isVerified)
      return res.status(403).json({ msg: 'Account not yet verified by validators' });
...
router.post('/refresh', async (req, res) => {
  try {
    ...
    const newToken = signAccess({ id: user._id, role: user.role, email: user.email });
...
router.post('/validator-login', loginLimiter, async (req, res) => {
  try {
    const { error, value } = loginSchema.validate(req.body);
    if (error) return res.status(400).json({ msg: error.details[0].message });

    const user = await User.findOne({ email: value.email, isValidator: true });
    if (!user) return res.status(400).json({ msg: 'No validator account found' });

    const isMatch = await bcrypt.compare(value.password, user.passwordHash);
    if (!isMatch) return res.status(400).json({ msg: 'Invalid credentials' });

    const token = signAccess({ id: user._id, role: user.role, email: user.email, isValidator: true });
    res.json({ token, user: { id: user._id, name: user.name, role: user.role, isValidator: true } });
  } catch (err) {
    res.status(500).json({ msg: err.message });
  }
});
...
// PUT /profile — name and educationInfo permanently locked
// freely editable: personalInfo (phone/address), skills, projects, experience
router.put('/profile', auth, async (req, res) => {
  try {
    const { personalInfo, professionalInfo, skills, projects, experience } = req.body;
...
// PUT /profile/resume — free resume re-upload, no validator needed
router.put('/profile/resume', auth, resumeUpload.single('resume'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ msg: 'No resume file provided' });
    const newResumePath = await uploadToSupabase(req.file);
    if (!newResumePath) return res.status(500).json({ msg: 'File upload failed' });
    await User.findByIdAndUpdate(req.user.id, { resumePath: newResumePath });
    res.json({ msg: 'Resume updated successfully', resumePath: newResumePath });
  } catch (err) {
    res.status(500).json({ msg: err.message });
  }
});
...
// POST /create-validator — protected by setup key
router.post('/create-validator', async (req, res) => {
  try {
    const { email, password, name, setupKey } = req.body;
    if (setupKey !== (process.env.VALIDATOR_SETUP_KEY || 'edunetchain-setup'))
    ...
    if (existing) {
      ...
      if (!existing.role || existing.role === 'student') existing.role = 'faculty';
      await existing.save();
